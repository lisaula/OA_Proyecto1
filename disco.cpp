...
//    }
    char array[20];
    strcpy(array,nombre.c_str());
    Node *n = new Node(array,0);
    if(tree.buscar(n)){
        return n->index_inarray;
    }
    return -1;
}
...
bool Disco::mount(string nombre)
{
    ...
    for(int i =0; i < sb.cantofinode;i++){
        ...
        if(temp.inode_index!=-1){
            Node *t = new Node(temp.name,temp.inode_index);
            tree.insertar(t);
        }
    }
...
//    //cout<<ft_array[0].name<<endl;
    ...
    //cout<<"ES bloque numero "<<FS_blockused-2<<" en uso "<<is_block_in_use(bitmap,FS_blockused-2)<<endl;
...
//                    array_ISpos =ocupados floor(ocupados-x);
//                }else{
//                    array_ISpos = floor(ocupados);
//                }
                array_ISpos = ocupados - x*(array_pos);
...
//                        }
                        //
                        int pos = getFreePosInArray(IS,x);
                        //cout<<"POS EN NUEVO "<<pos<<" array IS_pos"<<array_ISpos<<endl;
                        IS[pos] = blocksindex[0];
                        //cout<<IS[pos]<<endl;
                        move_to=blocksindex[0]*sb.sizeofblock;
                        //cout<<"Move to "<<move_to<<endl;
...
//    }
    char a[20];
    strcpy(a,nombre.c_str());
    Node* n = new Node(a,0);
    return tree.buscar(n);
}
...
bool Disco::mkDir(string nombre)
{
    ...
    //                t+=1;
            ...
            //cout<<"Escribiendo back "<<back.name<<endl;
...
bool Disco::cp(string nombre, string path, string new_name)
{
    if(nameExist(nombre) && nameExist(path) && !nameExist(new_name)){
        inode_d scr,dest;
        int num = seek(nombre);
        scr = seekInode(num,this->path);
        if(scr.permisos[0]!='d'){
            num = seek(path);
            dest = seekInode(num,this->path);
            if(dest.permisos[0]=='d'){
                inode_d temp = global;
                global = dest;
                global_pos =sizeof(superBlock_d)+(bitmap_size*sizeof(char))+(bit_inode_size*sizeof(char))+(sizeof(FileTable_d)*sb.cantofinode)
                        +(num*sizeof(inode_d));
